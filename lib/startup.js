...
function makeServer(config) {
  ...
  return P.promisify(server.register, server)(require('hapi-auth-jwt2')).then(function(err) {
    if (err) {
    log.error(err);
    }
    ...
    return P.promisify(server.register, server)(require('./../plugins/index')).then(function() {
      server.views({
        engines: {
